...
2. 默认使用「定期协整重算」更新对冲比率（滚动窗口 OLS + ADF）
3. 在回测过程中按指定窗口与间隔周期性复检协整关系并更新 beta
4. 提供多策略 Z-score 计算
5. 支持参数优化（网格/随机/贝叶斯）以及稳定性测试
6. RLS 递推更新保留为实验性选项（选项 2）
"""
...
def configure_trading_parameters():
    ...
    default_params = {
        ...
        'position_ratio': 0.5,
        'leverage': 5,
        'trading_fee_rate': 0.0005  # 交易手续费率 0.0275%
    }
...
class AdvancedCointegrationTrading:
    ...
    def close_position(self, pair_info, current_prices, reason, timestamp, current_spread):
        ...
        # 交易净盈亏 = 毛利 − 开仓费 − 平仓费（开仓费已在开仓时从 capital 扣除，资金变动见 capital_delta）
        open_fee = position.get('open_fee', 0)
        total_fee = open_fee + close_fee
        net_pnl = total_pnl - total_fee
        capital_delta = total_pnl - close_fee
            ...
            for pair_info in selected_pairs:
                ...
                if pair_info['pair_name'] in self.positions:
                    ...
                    if should_close:
                        ...
                        if trade:
                            capital += trade['capital_delta']  # +毛利 − 平仓费（开仓费已在开仓时扣除）
                            results['trades'].append(trade)
...
class ParameterOptimizer:
    ...
    def __init__(self, data, selected_pairs, initial_capital=10000,
        ...
        self.param_space = {
            ...
            'z_threshold': {'type': 'float', 'coarse': [1.0, 1.5, 2.0, 2.5, 3.0,3.5,4.5,5.0], 'fine_step': 0.1},
    ...
    def evaluate_params(self, params: Dict[str, Any], verbose: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            rls_max_change_rate = params.get('rls_max_change_rate', self.hedge_ratio_max_change_rate) if self.optimize_rls_params else self.hedge_ratio_max_change_rate
    ...
    def grid_search(self, coarse_first: bool = True,
                    ...
                    max_coarse_combinations: int = 1000) -> Dict[str, Any]:
        """
        网格搜索优化（分层：粗粒度 + 细粒度）
