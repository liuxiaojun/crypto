...
功能：
1. 实时数据获取和管理
2. 协整交易策略执行（多种 Z-score 策略）
3. 对冲比率更新：定期协整重算（推荐）/ RLS / 静态
4. 实时交易执行和监控
5. Web监控界面
6. 预热数据收集
"""
...
class BinanceAPI:
    ...
    def __init__(self, api_key=API_KEY, secret_key=SECRET_KEY, base_url=BASE_URL):
        ...
        # 是否双向持仓：None 表示尚未成功查询，True/False 为缓存结果（与 basic_binance_test 一致）
        self._dual_side_position_cached = None

    def is_dual_side_position(self):
        """
        查询 U 本位合约是否为「双向持仓」(Hedge)。
...
def main():
    """主函数"""
    print("实盘协整交易系统（定期协整重算 / RLS / 多种Z-score策略）")
    print("实时交易基础设施")
    print()
