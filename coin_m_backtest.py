...
# -*- coding: utf-8 -*-
"""
币本位（COIN-M）交割跨期协整回测与参数优化

相对 U 本位线性合约，本文件按币安反向合约规则记账：
- 保证金、手续费、盈亏均为结算币（BTC/ETH/SOL/BNB/XRP），不是 USDT
- 下单单位为「张」，BTC 1 张 = 100 USD，其余主流币 1 张 = 10 USD
- 盈亏：N × 面值 × (1/开仓价 - 1/平仓价) × 方向
- 适用于同币当季 vs 次季（两腿同一结算币）；跨不同币种的币本位对不做（钱包互不通用）
- 交割月：近月到期前默认 7 天禁开、3 天强制两腿平仓；两腿行情不再重叠且仍有仓时按最近价格平掉

流程与 U 本位版相同：外部筛完协整对后，本文件负责对冲比率、Z-score、回测与优化。
"""
...
# ==================== 币本位反向合约规则（币安 COIN-M） ====================
# 合约面值（USD / 张），与 dapi exchangeInfo.contractSize 一致
...
# 回测默认初始保证金（结算币数量，不是 USDT）
DEFAULT_COIN_M_CAPITAL = {
    'BTC': 1.0,
    'ETH': 10.0,
    'BNB': 5.0,
    'SOL': 50.0,
...
def parse_delivery_expiry_date(symbol: str):
    """BTCUSD_260925 -> date(2026, 9, 25)；永续或无法解析则 None。"""
    s = str(symbol).upper().replace('/', '').replace('-', '')
    if "_" not in s:
        return None
...
def coin_m_contract_size(symbol_or_base: str) -> float:
    base = parse_coin_m_base(symbol_or_base)
    return float(COIN_M_CONTRACT_SIZE.get(base, 10.0))


def default_coin_m_capital(symbol_or_base: str) -> float:
    base = parse_coin_m_base(symbol_or_base)
    return float(DEFAULT_COIN_M_CAPITAL.get(base, 1.0))


def inverse_pnl_coin(n_signed: float, contract_size: float, entry_price: float, exit_price: float) -> float:
    """
    反向合约盈亏（结算币）。
    n_signed > 0 为多，< 0 为空。
    PnL = N × 面值 × (1/开仓价 - 1/平仓价)
    """
...
def inverse_fee_coin(n_abs: float, contract_size: float, price: float, fee_rate: float) -> float:
    """手续费（结算币）= 币数量名义本金 × 费率，名义本金 = |N| × 面值 / 价格。"""
    if n_abs <= 0 or contract_size <= 0 or price <= 0:
        return 0.0
    return abs(n_abs) * contract_size / price * fee_rate


def inverse_margin_coin(n_abs: float, contract_size: float, price: float, leverage: float) -> float:
    """初始保证金（结算币）= |N| × 面值 / (价格 × 杠杆)。"""
    if n_abs <= 0 or contract_size <= 0 or price <= 0 or leverage <= 0:
        return 0.0
    return abs(n_abs) * contract_size / (price * leverage)


def fmt_coin(x: float, digits: int = 8) -> str:
...
def input_preselected_pairs(data, default_diff_order=0):
    ...
    print("  2. 请直接输入需要回测/优化的币对，例如: BTCUSD_260925/BTCUSD_261225")
    ...
    for pair_text in pair_candidates:
        ...
        if base1 != base2:
            print(
                f"   跳过 {symbol1}/{symbol2}：币本位两腿结算币不同（{base1} vs {base2}），"
                "保证金不能共用，本回测只做同币跨期。"
            )
            continue
        ...
        print(
            f"   已添加 {pair_info['pair_name']}，对冲比率 {hedge_ratio:.6f}，价差类型 {'原始' if pair_diff_order == 0 else '一阶差分'}")
...
def input_periodic_cointegration_params(avg_period_minutes, default_window_size=500, default_days=10):
    ...
    print(f"检测到数据周期: {avg_period_minutes:.1f} 分钟/根K线")
...
def input_cointegration_window_size(default_size=500):
    """仅输入滚动窗口大小（兼容旧接口，不含重算间隔）。"""
    print(f"定期协整滚动窗口大小 (默认{default_size}): ", end='')
    user_input = input().strip()
    if not user_input:
        return default_size
    try:
        window_size = int(user_input)
        return window_size if window_size > 0 else default_size
    except ValueError:
        return default_size
...
def configure_trading_parameters(settlement_asset='BTC', default_capital=1.0):
    """配置交易参数（资金单位为结算币，如 BTC，不是 USDT）"""
    print("\n" + "=" * 60)
    print("交易参数配置（币本位：保证金/盈亏按结算币）")
    print("=" * 60)
    ...
    print(f"结算币: {settlement_asset}（账户里要的是币，不是 USDT）")
    print("当前默认参数:")
    print(f"  0. 初始资金: {default_params['initial_capital']} {settlement_asset}")
    ...
    print(f"  4. 止盈百分比: {default_params['take_profit_pct'] * 100:.1f}%（相对币数量名义本金）")
    ...
    if modify_choice == 'y':
        ...
        capital_input = input(
            f"初始资金/{settlement_asset} (默认: {default_params['initial_capital']}): "
        ).strip()
        if capital_input:
            try:
                cap = float(capital_input)
                if cap <= 0:
                    print(f"初始资金必须为正，使用默认值: {default_params['initial_capital']}")
                else:
                    default_params['initial_capital'] = cap
            except ValueError:
                print(f"输入无效，使用默认值: {default_params['initial_capital']}")
...
def configure_hedge_ratio_update_mode():
    """
    选择对冲比率更新方式（仅模式选择；窗口/间隔由 input_periodic_cointegration_params 单独配置）

    Returns:
        dict: use_periodic_recalc, use_rls, rls_lambda
    """
    print("\n配置对冲比率更新方式")
...
class AdvancedCointegrationTrading:
    ...
    def __init__(self, lookback_period=60, z_threshold=2.0, z_exit_threshold=0.5,
                 ...
                 cointegration_window_size=500,
                 expiry_no_entry_days=7, expiry_force_close_days=3):
        """
        ...
        Args:
            ...
            cointegration_check_interval: 协整重算间隔（K线根数）
            data_period_minutes: 数据周期（分钟，用于显示，默认60分钟=1h）
            diff_order: 价差类型，0=原始价差，1=一阶差分价差
            cointegration_window_size: 定期协整滚动窗口大小（默认500）
            expiry_no_entry_days: 近月交割日前多少天禁止开新仓（默认7）
            expiry_force_close_days: 近月交割日前多少天强制两腿平仓（默认3）
        """
        ...
        self.trading_fee_rate = trading_fee_rate  # 交易手续费率（按币数量名义本金）
        self.expiry_no_entry_days = max(0, int(expiry_no_entry_days))
        self.expiry_force_close_days = max(0, int(expiry_force_close_days))
        self.maintenance_margin_rate = COIN_M_MAINTENANCE_MARGIN_RATE
        self.settlement_asset = 'COIN'
        self.positions = {}  # 当前持仓
        self.trades = []  # 交易记录
    ...
    def calculate_position_size_beta_neutral(self, margin_coin, price1, price2, hedge_ratio, signal,
        ...
        OLS 对冲仍是 q2 = β × q1（币数量），再换成张：
            N = q × price / contract_size
        初始保证金：
            IM = Σ |N| × 面值 / (价格 × 杠杆)
        """
    ...
    def _position_gross_pnl_coin(self, position, price1, price2):
        """两腿反向合约未扣费盈亏（结算币）。"""
        symbol1, symbol2 = position['symbol1'], position['symbol2']
        entry1 = position['entry_prices'][symbol1]
        entry2 = position['entry_prices'][symbol2]
        cs1 = position.get('contract_size1', coin_m_contract_size(symbol1))
        cs2 = position.get('contract_size2', coin_m_contract_size(symbol2))
        pnl1 = inverse_pnl_coin(position['symbol1_size'], cs1, entry1, price1)
        pnl2 = inverse_pnl_coin(position['symbol2_size'], cs2, entry2, price2)
        return pnl1, pnl2, pnl1 + pnl2

    def _position_close_fee_coin(self, position, price1, price2):
        symbol1, symbol2 = position['symbol1'], position['symbol2']
        cs1 = position.get('contract_size1', coin_m_contract_size(symbol1))
        cs2 = position.get('contract_size2', coin_m_contract_size(symbol2))
        return (
    ...
    def _position_maintenance_margin(self, position, price1, price2):
        symbol1, symbol2 = position['symbol1'], position['symbol2']
        cs1 = position.get('contract_size1', coin_m_contract_size(symbol1))
        cs2 = position.get('contract_size2', coin_m_contract_size(symbol2))
        notional_coin = (
            abs(position['symbol1_size']) * cs1 / price1
            + abs(position['symbol2_size']) * cs2 / price2
        )
        return notional_coin * self.maintenance_margin_rate
    ...
    def initialize_static_hedge_for_pair(self, pair_key, pair_info):
        """静态模式：使用输入对冲比率，不更新"""
        hedge_ratio = pair_info.get('hedge_ratio', 1.0)
        self.hedge_ratios[pair_key] = hedge_ratio
    ...
    def _pair_close_prices(self, symbol1, symbol2, current_prices, last_prices):
        """优先用本根价格，缺腿则用上一根有效价。"""
        prices = {}
        for symbol in (symbol1, symbol2):
            if symbol in current_prices:
                ...
                prices[symbol] = last_prices[symbol]
        return prices

    def _flatten_open_pair(self, pair_info, prices, timestamp, reason, capital, results):
        """按给定价格强制平掉该跨期仓（用于交割窗口或缺腿）。"""
        pair_name = pair_info['pair_name']
        if pair_name not in self.positions:
            return capital
        symbol1, symbol2 = pair_info['symbol1'], pair_info['symbol2']
        if symbol1 not in prices or symbol2 not in prices:
            if not (hasattr(self, '_optimization_mode') and self._optimization_mode):
                print(f"  {pair_name}: {reason}，但两腿都没有可用平仓价，仓位暂留")
            return capital
        pair_key = f"{symbol1}_{symbol2}"
        hedge_ratio = self.get_hedge_ratio_for_pair(pair_key, pair_info)
        current_spread = self.calculate_current_spread(
            prices[symbol1], prices[symbol2], hedge_ratio
        )
        trade = self.close_position(pair_info, prices, reason, timestamp, current_spread)
        if trade:
            capital += trade['pnl']
    ...
    def execute_trade(self, pair_info, current_prices, signal, timestamp, current_spread, available_capital):
        """
        执行交易（币本位：available_capital 为结算币保证金预算）
        """
        symbol1, symbol2 = pair_info['symbol1'], pair_info['symbol2']
        ...
        base1, base2 = parse_coin_m_base(symbol1), parse_coin_m_base(symbol2)
        if base1 != base2:
            print(f"开仓失败: 两腿结算币不同 {base1}/{base2}，币本位钱包不能共用")
            return None
        ...
        symbol1_size, symbol2_size, margin_used = self.calculate_position_size_beta_neutral(
            available_capital, price1, price2, hedge_ratio, signal, cs1, cs2
        )

        if symbol1_size is None:
            print(
                f"开仓失败: 保证金不足或张数不足 1 张 "
                f"(可用保证金: {fmt_coin(available_capital)} {self.settlement_asset})"
            )
            return None

        open_fee = (
            inverse_fee_coin(abs(symbol1_size), cs1, price1, self.trading_fee_rate)
            + inverse_fee_coin(abs(symbol2_size), cs2, price2, self.trading_fee_rate)
        )
        notional_coin = (
            abs(symbol1_size) * cs1 / price1
            + abs(symbol2_size) * cs2 / price2
        )
        ...
        if not (hasattr(self, '_optimization_mode') and self._optimization_mode):
            print(f"开仓: {pair_info['pair_name']}  [币本位 {base1}]")
            print(f"   信号: {signal['description']}")
            print(f"   价格: {symbol1}={price1:.4f}, {symbol2}={price2:.4f}")
            print(f"   价差: {current_spread:.6f}")
            print(
                f"   张数: {symbol1}={symbol1_size} 张(面值{cs1:g} USD), "
                f"{symbol2}={symbol2_size} 张(面值{cs2:g} USD)"
            )
            print(
                f"   初始保证金: {fmt_coin(margin_used)} {base1} / "
                f"预算 {fmt_coin(available_capital)} {base1}"
            )
            print(f"   开仓手续费: {fmt_coin(open_fee)} {base1}")

        return position

    def check_exit_conditions(self, pair_info, current_prices, current_z_score, timestamp, current_spread,
                              wallet_coin=None):
        """检查平仓条件（含止盈止损与币本位维持保证金强平）"""
        pair_name = pair_info['pair_name']
        if pair_name not in self.positions:
            return False, ""
        ...
        _, _, total_pnl = self._position_gross_pnl_coin(position, price1, price2)
        close_fee = self._position_close_fee_coin(position, price1, price2)
        net_pnl = total_pnl - close_fee

        if wallet_coin is not None:
            equity = wallet_coin + total_pnl
            mm = self._position_maintenance_margin(position, price1, price2)
            if equity <= mm:
                return True, (
                    f"币本位保证金不足强平"
                    f"(权益={fmt_coin(equity)} / 维持保证金={fmt_coin(mm)} "
                    f"{position.get('settlement_asset', self.settlement_asset)})"
                )
    ...
    def close_position(self, pair_info, current_prices, reason, timestamp, current_spread):
        ...
        pnl1, pnl2, total_pnl = self._position_gross_pnl_coin(position, price1, price2)
        close_fee = self._position_close_fee_coin(position, price1, price2)
        ...
        if not (hasattr(self, '_optimization_mode') and self._optimization_mode):
            print(f"平仓: {pair_name}  [币本位 {asset}]")
            print(f"   平仓原因: {reason}")
            print(f"   毛盈亏: {fmt_coin(total_pnl)} {asset} (腿1 {fmt_coin(pnl1)} / 腿2 {fmt_coin(pnl2)})")
            print(f"   开仓手续费: {fmt_coin(open_fee)} {asset}")
            print(f"   平仓手续费: {fmt_coin(close_fee)} {asset}")
            print(f"   总手续费: {fmt_coin(total_fee)} {asset}")
            print(f"   净盈亏: {fmt_coin(net_pnl)} {asset}")
            print(f"   持仓时间: {trade['holding_hours']:.1f}小时")
            print(f"   开仓价格: {symbol1}={entry_price1:.4f}, {symbol2}={entry_price2:.4f}")
            print(f"   平仓价格: {symbol1}={price1:.4f}, {symbol2}={price2:.4f}")
    ...
    def plot_equity_curve(self, capital_curve, save_path=None):
        ...
        ax1.set_title('资金曲线（结算币权益，含未实现盈亏）', fontsize=14, fontweight='bold')
        ax1.set_ylabel('权益（结算币）', fontsize=12)
    ...
    def backtest_cointegration_trading(self, data, selected_pairs, initial_capital=1.0):
        ...
        if not optimization_mode:
            print("\n" + "=" * 60)
            print("开始币本位（COIN-M 反向合约）协整交易回测")
            if self.use_arima_garch:
        ...
        # 初始化
        # 投入保证金（结算币）= 初始币数量 × 仓位比例
        capital = initial_capital * self.position_ratio
        settlement_asset = 'COIN'
        if selected_pairs:
            settlement_asset = parse_coin_m_base(selected_pairs[0]['symbol1'])
        ...
        if not optimization_mode:
            ...
            print(f"策略参数（币本位，单位={settlement_asset}）:")
            print(f"  初始资金: {fmt_coin(initial_capital)} {settlement_asset}")
            print(f"  仓位比例: {self.position_ratio * 100:.1f}%")
            print(f"  投入保证金: {fmt_coin(capital)} {settlement_asset} (初始资金 × 仓位比例)")
            print(f"  杠杆: {self.leverage:.1f}倍")
            print(f"  维持保证金率: {self.maintenance_margin_rate * 100:.2f}%（按币数量名义本金）")
            ...
            print(f"  近月到期前禁开仓: {self.expiry_no_entry_days}天")
            print(f"  近月到期前强制平仓: {self.expiry_force_close_days}天")
            if self.use_periodic_recalc:
                print(f"  对冲比率更新: 定期协整重算")
                ...
                print(f"  对冲比率更新: RLS递推（实验性）")
                print(f"  RLS遗忘因子: {self.rls_lambda}")
                print(f"  协整检验间隔: {self.cointegration_check_interval} 根K线 "
                      f"(约{self.cointegration_check_interval * self.data_period_minutes / 60 / 24:.1f}天)")
            else:
                print(f"  对冲比率更新: 静态（不更新）")
            # 显示使用的策略
            if self.z_score_strategy is not None:
        ...
        if self.use_periodic_recalc:
            ...
            for pair_info in selected_pairs:
                ...
                init_window_size = min(self.cointegration_window_size, len(data[symbol1]), len(data[symbol2]))
                ...
                try:
                    ...
                    if not optimization_mode:
                        print(f"  警告: {pair_key} 定期重算初始化失败: {e}")
        elif self.use_rls:
            if not optimization_mode:
                print("\n初始化RLS...")
            for pair_info in selected_pairs:
            ...
            for pair_info in selected_pairs:
                symbol1, symbol2 = pair_info['symbol1'], pair_info['symbol2']
                pair_key = f"{symbol1}_{symbol2}"
                self.initialize_static_hedge_for_pair(pair_key, pair_info)
        ...
        for i, timestamp in enumerate(all_timestamps):
            ...
            for symbol in data.keys():
                if timestamp in data[symbol].index:
                    ...
                    last_prices[symbol] = current_prices[symbol]

            # 检查每个选择的币对
            for pair_info in selected_pairs:
                symbol1, symbol2 = pair_info['symbol1'], pair_info['symbol2']
                pair_key = f"{symbol1}_{symbol2}"
                has_both = symbol1 in current_prices and symbol2 in current_prices
                has_pos = pair_info['pair_name'] in self.positions
                expiry_date = pair_near_expiry_date(symbol1, symbol2)
                days_left = calendar_days_to_expiry(timestamp, expiry_date)
                in_force_close = days_left is not None and days_left <= self.expiry_force_close_days
                in_no_entry = days_left is not None and days_left <= self.expiry_no_entry_days

                if has_pos and (in_force_close or not has_both):
                    close_prices = self._pair_close_prices(
                        symbol1, symbol2, current_prices, last_prices
                    )
                    if in_force_close:
                        if days_left is not None and days_left < 0:
                            reason = f"近月已交割，强制平仓(到期日{expiry_date})"
                        else:
                            reason = (
                                f"近月交割前强制平仓(到期日{expiry_date}，剩余{days_left}天)"
                            )
                    else:
                        reason = "两腿行情不再重叠，强制平仓"
                    capital = self._flatten_open_pair(
                        pair_info, close_prices, timestamp, reason, capital, results
                    )
                    continue
                ...
                # 协整未通过：强制平仓（若有持仓）并禁止开新仓
                if (self.use_periodic_recalc or self.use_rls) and pair_key in self.cointegration_status:
                    if not self.is_cointegration_trading_allowed(pair_key):
                        trade = self.force_close_on_cointegration_failure(
                            pair_info, current_prices, timestamp, current_hedge_ratio
                        )
                        if trade:
                            capital += trade['pnl']
                            results['trades'].append(trade)
                ...
                # 检查开仓条件（只有在没有持仓时才开仓）
                    ...
                    if in_no_entry or in_force_close:
                        continue
                    if not self.is_cointegration_trading_allowed(pair_key):
                        continue
                    ...
                    if signal['action'] != 'HOLD':
                        # 币本位：钱包余额即为保证金预算（不再把余额×杠杆当成 U 名义本金）
            ...
            # 记录资金曲线：钱包 + 未实现盈亏（反向合约盯市）
            unrealized = 0.0
            for pos in self.positions.values():
                s1, s2 = pos['symbol1'], pos['symbol2']
                if s1 in current_prices and s2 in current_prices:
                    _, _, upnl = self._position_gross_pnl_coin(
                        pos, current_prices[s1], current_prices[s2]
                    )
                    unrealized += upnl
        ...
        # 回测结束仍有持仓：用最后有效价平掉，避免未平仓污染胜率
        if self.positions and all_timestamps:
            last_ts = all_timestamps[-1]
            for pair_info in selected_pairs:
                if pair_info['pair_name'] not in self.positions:
                    continue
                symbol1, symbol2 = pair_info['symbol1'], pair_info['symbol2']
                close_prices = self._pair_close_prices(
                    symbol1, symbol2, {}, last_prices
                )
                capital = self._flatten_open_pair(
                    pair_info, close_prices, last_ts,
                    "回测结束强制平仓", capital, results
                )
            if results['capital_curve']:
                results['capital_curve'][-1]['capital'] = capital
                results['capital_curve'][-1]['wallet'] = capital
                results['capital_curve'][-1]['unrealized_pnl'] = 0.0
                results['capital_curve'][-1]['positions_count'] = len(self.positions)
        ...
        if results['capital_curve']:
            final_equity = results['capital_curve'][-1]['capital']
        if initial_invested_capital > 0:
            final_return = (final_equity - initial_invested_capital) / initial_invested_capital * 100
        else:
            final_return = 0.0
        ...
        if not optimization_mode:
            print(f"\n回测结果（结算币 {settlement_asset}）:")
            print(f"  初始资金: {fmt_coin(initial_capital)} {settlement_asset}")
            print(f"  投入保证金: {fmt_coin(initial_invested_capital)} {settlement_asset} (初始资金 × 仓位比例)")
            print(f"  最终权益: {fmt_coin(final_equity)} {settlement_asset}")
            ...
            print(f"  总手续费: {fmt_coin(total_fees)} {settlement_asset}")

            print(f"\n风险指标:")
            print(f"  最大回撤: {fmt_coin(risk_metrics.get('max_drawdown', 0))} {settlement_asset}")
            ...
            print(f"  平均盈利: {fmt_coin(risk_metrics.get('avg_profit', 0))} {settlement_asset}")
            print(f"  平均亏损: {fmt_coin(risk_metrics.get('avg_loss', 0))} {settlement_asset}")
        ...
        try:
            ...
            for i, trade in enumerate(trades):
                ...
                row = {
                    ...
                    '币种1张数': trade.get('symbol1_size', 0),
                    '币种2张数': trade.get('symbol2_size', 0),
                ...
                if trade.get('action') == 'OPEN':
                    ...
                    row['使用保证金(结算币)'] = trade.get('capital_used', 0)
                ...
                # 平仓特有信息
                    ...
                    row['使用保证金(结算币)'] = trade.get('capital_used', '')
...
def test_rolling_window_cointegration_trading(csv_file_path):
    ...
    if mode_params['use_periodic_recalc'] or mode_params['use_rls']:
        print("\n5. 配置定期协整参数（窗口、重算间隔）")
        periodic_params = input_periodic_cointegration_params(avg_period_minutes)
    hedge_params = build_hedge_params(mode_params, periodic_params, avg_period_minutes)
    ...
    while True:
        ...
        settle = parse_coin_m_base(selected_pairs[0]['symbol1'])
        trading_params = configure_trading_parameters(
            settlement_asset=settle,
            default_capital=default_coin_m_capital(settle),
        )

        # 9. 执行交易回测（对冲比率模式已在步骤4配置，此处不再重复询问）
        ...
        results = trading_strategy.backtest_cointegration_trading(
            data,
            selected_pairs,
            initial_capital=trading_params.get('initial_capital', default_coin_m_capital(settle))
        )
        ...
        if results['trades']:
            ...
            for pair, trades in pair_trades.items():
                ...
                for trade in trades:
                    ...
                    asset = trade.get('settlement_asset', settle)
                    if trade['action'] == 'CLOSE':
                        pnl_info = f", 盈亏: {fmt_coin(trade.get('pnl', 0))} {asset}"
                    else:
                        n1, n2 = trade.get('symbol1_size', 0), trade.get('symbol2_size', 0)
                        pnl_info = f", 张数: {n1}/{n2}"
                    print(
...
class ParameterOptimizer:
    ...
    def __init__(self, data, selected_pairs, initial_capital=1.0,
                 objective='sharpe_ratio', stability_test=True, z_score_strategy=None,
                 cointegration_window_size=500, diff_order=0,
                 use_periodic_recalc=True, use_rls=False,
                 hedge_ratio_max_change_rate=0.2, rls_lambda=0.99,
                 cointegration_check_interval=240, data_period_minutes=60,
                 optimize_zscore_params=True, optimize_rls_params=False):
        """
        初始化参数优化器
        ...
        Args:
            ...
            cointegration_window_size: 定期协整滚动窗口大小（默认500）
            diff_order: 价差类型，0=原始价差，1=一阶差分价差
            use_periodic_recalc: 是否使用定期协整重算（默认True）
            use_rls: 是否使用RLS递推更新（实验性，默认False）
            hedge_ratio_max_change_rate: 对冲比率最大单次变化率
            rls_lambda: RLS遗忘因子
            cointegration_check_interval: 协整重算间隔（K线根数）
            data_period_minutes: 数据周期（分钟，用于显示，默认60）
            optimize_zscore_params: 是否优化Z-score策略参数（默认True）
            optimize_rls_params: 是否优化RLS参数（仅 use_rls=True 时有效，默认False）
        """
        self.data = data
        ...
        self.optimize_rls_params = optimize_rls_params and self.use_rls
        ...
        # 根据是否使用RLS添加RLS参数（仅实验性 RLS 模式）
        if self.optimize_rls_params:
    ...
    def _add_zscore_strategy_params(self):
        ...
        if strategy_type == 'ArimaGarchZScoreStrategy':
            ...
            # Kalman Filter参数
            self.param_space['kalman_process_variance'] = {'type': 'float', 'coarse': [0.005, 0.01, 0.02, 0.05],
                                                           'fine_step': 0.005}
            self.param_space['kalman_observation_variance'] = {'type': 'float', 'coarse': [0.05, 0.1, 0.2, 0.5],
                                                               'fine_step': 0.05}
            ...
            self.param_space['copula_type'] = {'type': 'categorical', 'coarse': ['gaussian', 'student'],
                                               'fine_step': None}
    ...
    def _add_rls_params(self):
        ...
        self.param_space['rls_max_change_rate'] = {'type': 'float', 'coarse': [0.1, 0.15, 0.2, 0.25, 0.3],
                                                   'fine_step': 0.05}
    ...
    def evaluate_params(self, params: Dict[str, Any], verbose: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取RLS参数（仅 RLS 模式）
            rls_lambda = params.get('rls_lambda', self.rls_lambda) if self.optimize_rls_params else self.rls_lambda
            rls_max_change_rate = params.get('rls_max_change_rate',
                                             self.hedge_ratio_max_change_rate) if self.optimize_rls_params else self.hedge_ratio_max_change_rate
            use_rls = self.use_rls
            if 'use_rls' in self.param_space:
                use_rls = params.get('use_rls', self.use_rls)
            ...
            strategy = AdvancedCointegrationTrading(
                ...
                use_periodic_recalc=self.use_periodic_recalc,
                use_rls=use_rls,
                rls_lambda=rls_lambda,
                hedge_ratio_max_change_rate=rls_max_change_rate,
                ...
                if param_name in self.param_space:
                    ...
                    if space_def['type'] == 'categorical':
                        ...
                        if len(possible_values) > 1:
                            perturbed_params[param_name] = random.choice(
                                [v for v in possible_values if v != param_value])
                        # 如果只有一个值，保持不变
                        ...
                        if param_name == 'lookback_period':
                            ...
                            if self.diff_order == 1:
                                ...
                                perturbed_params[param_name] = max(0,
                                                                   min(1, perturbed_params[param_name]))  # 原始价差时，d最多为1
...
def test_parameter_optimization(csv_file_path):
    ...
    # 9. 创建优化器（对冲比率模式已在步骤3-4配置）
    strategy_name = z_score_strategy.get_strategy_description() if z_score_strategy else "未知"
    diff_type = '原始价差' if diff_order == 0 else '一阶差分价差'
    print(f"\n9. 创建优化器 (方法={method}, 目标={objective}, 策略={strategy_name}, 价差类型={diff_type})")
    if hedge_params['use_periodic_recalc'] or hedge_params['use_rls']:
        interval_days = hedge_params['cointegration_check_interval'] * avg_period_minutes / 60 / 24
        print(f"  定期协整滚动窗口: {hedge_params['cointegration_window_size']} 根K线")
        print(f"  定期协整重算间隔: {hedge_params['cointegration_check_interval']} 根K线 (约{interval_days:.1f}天)")
    if hedge_params['use_periodic_recalc']:
        print("  对冲比率更新: 定期协整重算")
    elif hedge_params['use_rls']:
        print("  对冲比率更新: RLS递推（实验性）")
    ...
    optimizer = ParameterOptimizer(
        ...
        initial_capital=default_coin_m_capital(selected_pairs[0]['symbol1']),
        objective=objective,
        stability_test=True,
        z_score_strategy=z_score_strategy,
        cointegration_window_size=hedge_params['cointegration_window_size'],
...
def main():
    ...
    主函数
    """
    print("币本位（COIN-M）协整跨期回测与参数优化")
    print("核心流程：")
    print("  0. 账户按结算币（BTC/ETH 等）计保证金与盈亏，下单单位为张")
    print("  1. 用户输入已筛选的协整币对及价差类型")
    print("  2. 默认使用定期协整重算（滚动窗口 OLS + ADF）更新对冲比率")
    print("  3. 回测期间按间隔周期性复检协整状态并调整仓位")
    print("  4. 可选参数优化（网格、随机、贝叶斯）与稳定性测试")
    print("  5. RLS 递推更新保留为实验性选项")
